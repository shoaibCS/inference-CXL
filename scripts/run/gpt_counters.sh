...
PERF="/home/shoaib/linux/tools/perf/perf"
EMON="/opt/intel/oneapi/vtune/2021.1.2/bin64/emon"
RUNDIR="/home/shoaib/run"
...
run_one_exp()
{
	...
	numactl --membind 0 /usr/bin/vmtouch -f -t /home/shoaib/inference-CXL/language/gpt-j/model/gpt-j/checkpoint-final
	
    elif [[ $et == "L0" ]]; then
	if [[ $id == "1" ]]; then
                run_cmd="numactl --cpunodebind 0 --membind 1 -- ""${run_cmd}"
		numactl --membind 1 /usr/bin/vmtouch -f -t /home/shoaib/inference-CXL/language/gpt-j/model/gpt-j/checkpoint-final
        else
                run_cmd="numactl --cpunodebind 0 --membind 2 -- ""${run_cmd}"
		numactl --membind 2 /usr/bin/vmtouch -f -t /home/shoaib/inference-CXL/language/gpt-j/model/gpt-j/checkpoint-final
        fi
...
#	numactl --membind 1 /usr/bin/vmtouch -f -t /home/shoaib/inference-CXL/language/gpt-j/model/gpt-j/checkpoint-final
	...
	perf_events="${perf_events}"",MEM_INST_RETIRED.ALL_LOADS,MEM_INST_RETIRED.ALL_STORES"
        ...
        perf_events="${perf_events}"",EXE_ACTIVITY.BOUND_ON_LOADS,EXE_ACTIVITY.BOUND_ON_STORES,CYCLE_ACTIVITY.STALLS_TOTAL"
        perf_events="${perf_events}"",MEMORY_ACTIVITY.STALLS_L1D_MISS,MEMORY_ACTIVITY.STALLS_L2_MISS,MEMORY_ACTIVITY.STALLS_L3_MISS"
        ...
        perf_events="${perf_events}"",cpu/OFFCORE_REQUESTS_OUTSTANDING.ALL_DATA_RD,cmask=4/,OFFCORE_REQUESTS_OUTSTANDING.CYCLES_WITH_DATA_RD"
        perf_events="${perf_events}"",OFFCORE_REQUESTS_OUTSTANDING.ALL_DATA_RD,L1D_PEND_MISS.PENDING,L1D_PEND_MISS.PENDING_CYCLES"
        perf_events="${perf_events}"",OFFCORE_REQUESTS_OUTSTANDING.DEMAND_DATA_RD,OFFCORE_REQUESTS.DEMAND_DATA_RD,OFFCORE_REQUESTS_OUTSTANDING.CYCLES_WITH_DEMAND_DATA_RD"
        perf_events="${perf_events}"",OFFCORE_REQUESTS.DATA_RD,OCR.DEMAND_RFO.ANY_RESPONSE"
        perf_events="${perf_events}"",OCR.DEMAND_RFO.L3_MISS,OCR.DEMAND_RFO.DRAM,OCR.DEMAND_RFO.L3_HIT"
        perf_events="${perf_events}"",OFFCORE_REQUESTS_OUTSTANDING.CYCLES_WITH_DEMAND_RFO"
        perf_events="${perf_events}"",OCR.DEMAND_DATA_RD.ANY_RESPONSE,OCR.DEMAND_DATA_RD.L3_MISS,OCR.DEMAND_DATA_RD.L3_HIT"
        perf_events="${perf_events}"",OCR.DEMAND_CODE_RD.ANY_RESPONSE,OCR.DEMAND_CODE_RD.DRAM"
        perf_events="${perf_events}"",OCR.HWPF_L1D.ANY_RESPONSE,OCR.HWPF_L2.ANY_RESPONSE"
        perf_events="${perf_events}"",XQ.FULL_CYCLES,L1D_PEND_MISS.L2_STALLS"
