...
source /home/shoaib/run/run-globals.sh
...
monitor_freq()
{
    while true; do
#        local o=$(sudo numactl --hardware)
	local unc_min_freq=$( cat /sys/devices/system/cpu/intel_uncore_frequency/package_00_die_00/min_freq_khz)
	local min_freq=$( cat /sys/devices/system/cpu/cpufreq/policy10/cpuinfo_min_freq)
	local unc_max_freq=$( cat /sys/devices/system/cpu/intel_uncore_frequency/package_00_die_00/max_freq_khz)
	local unc_cur_freq=$( cat /sys/devices/system/cpu/intel_uncore_frequency/package_00_die_00/current_freq_khz)
	local max_freq=$( cat /sys/devices/system/cpu/cpufreq/policy10/cpuinfo_max_freq)
	local scaling_freq0=$( cat /sys/devices/system/cpu/cpufreq/policy0/scaling_cur_freq)
	local scaling_freq2=$( cat /sys/devices/system/cpu/cpufreq/policy2/scaling_cur_freq)
	local scaling_freq4=$( cat /sys/devices/system/cpu/cpufreq/policy4/scaling_cur_freq)
	local scaling_freq6=$( cat /sys/devices/system/cpu/cpufreq/policy6/scaling_cur_freq)
	local scaling_freq8=$( cat /sys/devices/system/cpu/cpufreq/policy8/scaling_cur_freq)
	local scaling_freq10=$( cat /sys/devices/system/cpu/cpufreq/policy10/scaling_cur_freq)
	local scaling_freq12=$( cat /sys/devices/system/cpu/cpufreq/policy12/scaling_cur_freq)
	local scaling_freq14=$( cat /sys/devices/system/cpu/cpufreq/policy14/scaling_cur_freq)
	local scaling_freq16=$( cat /sys/devices/system/cpu/cpufreq/policy16/scaling_cur_freq)
	local scaling_freq18=$( cat /sys/devices/system/cpu/cpufreq/policy18/scaling_cur_freq)
	local scaling_freq20=$( cat /sys/devices/system/cpu/cpufreq/policy20/scaling_cur_freq)
	local scaling_freq22=$( cat /sys/devices/system/cpu/cpufreq/policy22/scaling_cur_freq)
	local scaling_freq24=$( cat /sys/devices/system/cpu/cpufreq/policy24/scaling_cur_freq)
	local scaling_freq26=$( cat /sys/devices/system/cpu/cpufreq/policy26/scaling_cur_freq)
	local scaling_freq28=$( cat /sys/devices/system/cpu/cpufreq/policy28/scaling_cur_freq)
	local scaling_freq30=$( cat /sys/devices/system/cpu/cpufreq/policy30/scaling_cur_freq)
	
	echo "------------------------------------------------------------------"
	echo " "
	echo "uncore min: $unc_min_freq -- uncore max: $unc_max_freq -- uncore current: $unc_cur_freq "
	printf "\nFrequencies of even cores, 0,2,4...30 \n0: $scaling_freq0\n2: $scaling_freq2\n4: $scaling_freq4\n6: $scaling_freq6\n8: $scaling_freq8\n10: $scaling_freq10\n12: $scaling_freq12\n14: $scaling_freq14\n16: $scaling_freq16\n18: $scaling_freq18\n20: $scaling_freq20\n22: $scaling_freq22\n24: $scaling_freq24\n26: $scaling_freq26\n28: $scaling_freq28\n30: $scaling_freq30\n"
 #       local node0_free_mb=$(echo "$o" | grep "node 0 free" | awk '{print $4}')
  #              local node1_free_mb=$(echo "$o" | grep "node 1 free" | awk '{print $4}')
   #     echo "$(date +"%D %H%M%S") ${node0_free_mb} ${node1_free_mb}"
        #pidstat -r -u -d -l -p ALL -U -h 5 100000000 > pidstat.log &
